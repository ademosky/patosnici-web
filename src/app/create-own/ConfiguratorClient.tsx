...
export default function ConfiguratorClient({ initialVehicles }: { initialVehicles: Vehicle[] }) {
  ...
  const configSummary = config.vehicle
    ...
    : "";
  const productTitle = `Платнени патосници — Изработи сам (${config.vehicle ? `${config.vehicle.generation}` : ""})`;
              ...
              {config.vehicle && (
                <div className="mt-6 space-y-3">
                  ...
                  <div className="space-y-2 rounded-xl border border-zinc-800 bg-[#1a1a1a] p-4">
                    <div className="flex justify-between text-sm"><span className="text-zinc-400">{lang === "sq" ? "Automjeti" : "Возило"}</span><span className="font-semibold text-white">{config.vehicle.generation}</span></div>
